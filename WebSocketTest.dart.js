...
// ********** Code for WebSocketTest **************
function WebSocketTest() {
  this.wsUri = "ws://echo.websocket.org";
}
WebSocketTest.prototype.run = function() {
  var $this = this; // closure support
  this.inputUri = get$$document().query("#wsUri");
  this.checkedSecure = get$$document().query("#secureCb");
  this.connect = get$$document().query("#connect");
  this.disconnect = get$$document().query("#disconnect");
  this.send = get$$document().query("#send");
  this.sendMessage = get$$document().query("#sendMessage");
  this.clearLog = get$$document().query("#clearLogBut");
  this.consoleLog = get$$document().query("#consoleLog");
  this.inputUri.value = this.wsUri;
  this.disconnect.disabled = true;
  this.checkedSecure.get$on().get$click().add((function (event) {
    if ($this.checkedSecure.checked) {
  ...
  , false);
  this.connect.get$on().get$click().add$1((function (e) {
    print$("connecting");
    $this.consoleLog.get$elements().add(_ElementFactoryProvider.Element$html$factory("<p>connecting</p>"));
    $this.webSocket = _WebSocketFactoryProvider.WebSocket$factory($this.wsUri);
    $this.webSocket.get$on().get$open().add((function (o) {
      $this.consoleLog.get$elements().add(_ElementFactoryProvider.Element$html$factory("<p>opened</p>"));
    ...
    , false);
    $this.webSocket.get$on().get$close().add((function (c) {
      $this.consoleLog.get$elements().add(_ElementFactoryProvider.Element$html$factory("<p>disconnecting</p>"));
      $this.disconnect.disabled = true;
    ...
    , false);
    $this.webSocket.get$on().get$message().add((function (m) {
      return $this.consoleLog.get$elements().add(_ElementFactoryProvider.Element$html$factory(("<p>" + m.data + "</p>")));
    })
    , false);
    $this.webSocket.get$on().get$error().add((function (er) {
      return $this.consoleLog.get$elements().add(_ElementFactoryProvider.Element$html$factory(("<p>" + er + "</p>")));
    })
    , false);
  })
  );
  this.disconnect.get$on().get$click().add$1((function (_) {
    return $this.webSocket.close();
  })
  );
  this.send.get$on().get$click().add$1((function (_) {
    if ($this.webSocket.readyState == (1)) {
      $this.webSocket.send($this.sendMessage.value);
    }
  })
  );
  this.clearLog.get$on().get$click().add$1((function (_) {
    return $this.consoleLog.get$elements().clear();
  })
  );
}
// ********** Code for top level **************
